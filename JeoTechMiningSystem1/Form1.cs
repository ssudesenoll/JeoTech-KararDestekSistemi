...
namespace JeoTechMiningSystem1
{
    public partial class Form1 : Form
    {
        ...
        private class HelmetData
        {
            ...
            public string ExpectedNextNodeId { get; set; } = "";
        ...
        private void InitializeComponentInternal()
        {
            ...
            Panel panelTop = new Panel { Dock = DockStyle.Top, Height = 80, BackColor = Color.FromArgb(20, 30, 45) };
            ...
            PictureBox pbLogo = new PictureBox
            {
                ...
                ImageLocation = @"C:\Users\snlsu\Downloads\Gemini_Generated_Image_y7c4tey7c4tey7c4-Photoroom-Photoroom.png",
            ...
            Panel panelLeft = new Panel { Dock = DockStyle.Left, Width = 410, BackColor = Color.FromArgb(18, 25, 40), Padding = new Padding(10) };
            this.Controls.Add(panelLeft);

            Label lblLeftTitle = new Label { Text = "OTONOM YÖNLENDİRME PANELİ", Font = new Font("Segoe UI", 10, FontStyle.Bold), ForeColor = Color.Cyan, Location = new Point(10, 15), AutoSize = true };
            ...
            _cmbLeftHelmets = new ComboBox { Location = new Point(165, yPos - 3), Width = 225, DropDownStyle = ComboBoxStyle.DropDownList, BackColor = Color.FromArgb(30, 40, 55), ForeColor = Color.White, Font = new Font("Segoe UI", 9, FontStyle.Bold) };
            ...
            Panel pnlDirectionBox = new Panel { Location = new Point(10, yPos + 30), Size = new Size(380, 110), BackColor = Color.FromArgb(25, 38, 55), BorderStyle = BorderStyle.FixedSingle };
            panelLeft.Controls.Add(pnlDirectionBox);

            pnlDirectionBox.Controls.Add(new Label { Text = "AKILLI BARET YÖN KOMUTU:", ForeColor = Color.Gold, Font = new Font("Segoe UI", 9, FontStyle.Bold), Location = new Point(10, 6), AutoSize = true });
            _lblNextDirection = new Label { Text = "SABİT (KOMUT YOK)", ForeColor = Color.Cyan, Font = new Font("Segoe UI", 12, FontStyle.Bold), Location = new Point(10, 25), AutoSize = true };
            pnlDirectionBox.Controls.Add(_lblNextDirection);

            int boxW = 85, boxH = 40, startX = 10, startY = 55;
            _lblDirLeft = CreateDirIndicator("⬅️ SOL", new Point(startX, startY), boxW, boxH);
            _lblDirForward = CreateDirIndicator("⬆️ DÜZ", new Point(startX + boxW + 5, startY), boxW, boxH);
            _lblDirRight = CreateDirIndicator("➡️ SAĞ", new Point(startX + (boxW + 5) * 2, startY), boxW, boxH);
            _lblDirStop = CreateDirIndicator("🛑 DUR", new Point(startX + (boxW + 5) * 3, startY), boxW, boxH);

            pnlDirectionBox.Controls.Add(_lblDirLeft);
            pnlDirectionBox.Controls.Add(_lblDirForward);
            pnlDirectionBox.Controls.Add(_lblDirRight);
            pnlDirectionBox.Controls.Add(_lblDirStop);
            ...
            yPos += 42;
            panelLeft.Controls.Add(new Label { Text = "✅ GÜVENLİ BÖLGEYE ULAŞANLAR", ForeColor = Color.LimeGreen, Font = new Font("Segoe UI", 9, FontStyle.Bold), Location = new Point(10, yPos), AutoSize = true });
            ...
            FlowLayoutPanel pnlInjector = new FlowLayoutPanel { Dock = DockStyle.Fill, Padding = new Padding(5), FlowDirection = FlowDirection.TopDown, WrapContents = false };
            tlpBottom.Controls.Add(pnlInjector, 2, 0);

            pnlInjector.Controls.Add(new Label { Text = "SİSTEM KONTROL VE TEST PANELİ", ForeColor = Color.Gold, Font = new Font("Segoe UI", 9, FontStyle.Bold), AutoSize = true, Margin = new Padding(0, 0, 0, 10) });

            _pnlSimControls = new Panel { Size = new Size(250, 115), Margin = new Padding(0) };
            ...
            _cmbTestBaret = new ComboBox { Location = new Point(95, 5), Width = 50, DropDownStyle = ComboBoxStyle.DropDownList };
            pnlBaretControls.Controls.Add(_cmbTestBaret);

            Button btnMove = new Button { Text = "Bareti Taşı", BackColor = Color.RoyalBlue, ForeColor = Color.White, FlatStyle = FlatStyle.Flat, Location = new Point(155, 4), Size = new Size(90, 24) };
            btnMove.Click += (s, e) => {
                if (_cmbLeftHelmets.SelectedItem is HelmetData h && _cmbTestBaret.SelectedItem != null)
                {
                    ...
                    else
                    {
                        ...
                        {
                            h.IsEvacuating = true;
                            if (!_evacuationTimer.Enabled) _evacuationTimer.Start();
                        }
                        else
                        {
                            h.IsEvacuating = false;
            ...
            pnlBaretControls.Controls.Add(btnMove);

            Button btnResetAll = new Button { Text = "🔄 Sistemi Sıfırla (Normale Dön)", BackColor = Color.SeaGreen, ForeColor = Color.White, FlatStyle = FlatStyle.Flat, Location = new Point(5, 40), Size = new Size(240, 26) };
            ...
            pnlBaretControls.Controls.Add(btnResetAll);

            Button btnExportCsv = new Button { Text = "📊 Raporu Dışa Aktar (CSV)", BackColor = Color.Teal, ForeColor = Color.White, FlatStyle = FlatStyle.Flat, Location = new Point(5, 75), Size = new Size(240, 26) };
            btnExportCsv.Click += (s, e) => {
                SaveFileDialog sfd = new SaveFileDialog { Filter = "CSV Dosyası (*.csv)|*.csv", FileName = $"Maden_Kaza_Raporu_{DateTime.Now:yyyyMMdd_HHmm}.csv" };
                if (sfd.ShowDialog() == DialogResult.OK)
                {
                    File.WriteAllLines(sfd.FileName, _csvLogData, Encoding.UTF8);
                    MessageBox.Show("Kaza raporu ve sensör geçmişi başarıyla kaydedildi.", "Rapor Hazır", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
            };
        ...
        private void PlaySiren()
        {
            if (!_isSirenPlaying)
            {
                try
                {
                    if (File.Exists(@"C:\Windows\Media\Alarm01.wav"))
                    {
                        _sirenPlayer = new SoundPlayer(@"C:\Windows\Media\Alarm01.wav");
        ...
        private void StopSiren()
        {
            if (_isSirenPlaying)
            {
                try { if (_sirenPlayer != null) _sirenPlayer.Stop(); } catch { }
                _isSirenPlaying = false;
            }
        }

        private Label CreateDirIndicator(string text, Point loc, int w, int h)
        {
            return new Label
            {
                Text = text,
                Location = loc,
                Size = new Size(w, h),
                BackColor = Color.FromArgb(35, 48, 70),
        ...
        private void HighlightDirectionIndicator(string activeDir)
        {
            ResetDirIndicator(_lblDirLeft);
            ResetDirIndicator(_lblDirForward);
            ResetDirIndicator(_lblDirRight);
            ResetDirIndicator(_lblDirStop);

            if (activeDir == "SOLA DÖN") ActivateDirIndicator(_lblDirLeft, Color.Cyan);
            else if (activeDir == "DÜZ DEVAM ET") ActivateDirIndicator(_lblDirForward, Color.LimeGreen);
            else if (activeDir == "SAĞA DÖN") ActivateDirIndicator(_lblDirRight, Color.Orange);
            else if (activeDir == "DUR") ActivateDirIndicator(_lblDirStop, Color.Red);
        }

        private void ResetDirIndicator(Label lbl)
        {
            lbl.BackColor = Color.FromArgb(35, 48, 70);
            lbl.ForeColor = Color.Gray;
        }

        private void ActivateDirIndicator(Label lbl, Color c)
        {
            lbl.BackColor = c;
        ...
        {
            _csvLogData.Add("Tarih_Saat;Mesaj_Icerigi");
        ...
        private void SystemTimer_Tick(object sender, EventArgs e)
        {
            foreach (var h in _helmetsList)
            {
                ...
                {
                    int ofkDrop = (h.HeartRate >= 120) ? 30 : 15;
                    h.OfkSecondsRemaining -= ofkDrop;
            ...
            foreach (var h in _helmetsList)
            {
                if (h.IsEvacuated)
                {
                    ...
                    if (selectedLeftPanelHelmet != null && selectedLeftPanelHelmet.Id == h.Id)
                    {
                        ...
                        _lblNextDirection.Text = "DUR (BAŞARILI)";
                        _lblNextDirection.ForeColor = Color.LimeGreen;
                        HighlightDirectionIndicator("DUR");
                ...
                if (selectedLeftPanelHelmet != null && selectedLeftPanelHelmet.Id == h.Id)
                {
                    ...
                    if (h.IsFallen)
                    {
                        ...
                        _lblNextDirection.Text = "DUR (HAREKETSİZ)";
                        _lblNextDirection.ForeColor = Color.Red;
                        HighlightDirectionIndicator("DUR");
                    ...
                    else if (h.IsTrapped)
                    {
                        ...
                        _lblNextDirection.Text = "DUR (GÜVENLİ BEKLE)";
                    ...
                    else if (result.Success)
                    {
                        ...
                        if (result.Route.Count > 1)
                        {
                            ...
                            MineNode prev = string.IsNullOrEmpty(h.PreviousNodeId) ? null : _graph.Nodes[h.PreviousNodeId];

                            h.ExpectedNextNodeId = next.Id;

                            string direction = DetermineDirection(prev, current, next);

                            if (h.EdgeProgress > 0.15 && direction != "DUR")
                            {
                                direction = "DÜZ DEVAM ET";
                            }

                            _lblNextDirection.Text = direction;
                            _lblNextDirection.ForeColor = direction == "DUR" ? Color.Red : Color.Cyan;
                            HighlightDirectionIndicator(direction);
                            ...
                            if (_isRealHardwareMode && h.Id == "B-01")
                            {
                                ...
                                else if (direction == "DÜZ DEVAM ET") SendGuidanceToHardware("VIB:FORWARD");
                                else if (direction == "DUR") SendGuidanceToHardware("VIB:STOP");
                            }
                        }
                        else
                        {
                            _lblNextDirection.Text = "DUR (HEDEFTE)";
                            _lblNextDirection.ForeColor = Color.LimeGreen;
                            HighlightDirectionIndicator("DUR");
                            _lblNextNode.Text = "-";
                        }
